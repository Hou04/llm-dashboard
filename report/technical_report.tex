...
\fancyfoot[C]{\thepage}
\fancyfoot[LE,RO]{\small LLM Dashboard — Technical Report v2.0}
...
{\small Department of Software Architecture \& AI Engineering}
...
\hline
1.0 & March 2026 & Architecture Team & Initial system specification \\
\hline
1.5 & March 2026 & Architecture Team & Added M8, M9 specifications \\
\hline
2.0 & April 2026 & Architecture Team & Full reverse-engineering and audit. Added M10 Billing, security review, ML pipeline documentation, UML diagrams \\
\hline
\end{tabularx}
...
\hline
Solution Architect & System design, module boundaries, event flows \\
\hline
Data Engineer & Database schemas, TimescaleDB, aggregation pipelines \\
\hline
ML Engineer & Anomaly detection, forecasting algorithms, LLM integration \\
\hline
Security Lead & Authentication, RBAC, data governance \\
\hline
\end{tabularx}
...
\hline
\texttt{call.logged} & M1 Gateway & M3 Detection & Every API call \\
\hline
...
\subsection{Architecture Positioning}
M2 occupies the \textbf{read-optimized data layer} between the raw time-series store and all consumer modules (M9 Dashboard, M10 Billing, M6 Forecasting). It reads from \texttt{llm\_token\_log} (produced by M1) and writes to pre-aggregated tables (\texttt{llm\_cost\_daily}, \texttt{llm\_cost\_monthly}). It also manages versioned pricing rules that determine how raw token counts translate to monetary costs. Importantly, M2 has no event bus subscriptions — it is purely batch-driven via Celery and query-driven via REST.

\subsection{Functional Scope}

\begin{itemize}
  \item \textbf{Nightly Aggregation} — A Celery task running at 02:00 UTC queries the previous day's token logs and upserts one row per tenant into \texttt{llm\_cost\_daily}. Uses INSERT ON CONFLICT UPDATE for idempotent re-runs.

  \item \textbf{Monthly Rollup} — On the first of each month at 03:00 UTC, a Celery task aggregates daily records into \texttt{llm\_cost\_monthly}, computing month-over-month cost and token change percentages.
...
The platform successfully ingested and simulated six months of synthetic historical operational data spanning from January 2025 to June 2025 across three distinct organizational tenants (\texttt{C1\_SMALL}, \texttt{C2\_MEDIUM}, and \texttt{C3\_ENTERPRISE}). 

A total of \textbf{243,657} distinct token log events were compiled and processed without any pipeline failures. Validation checkpoints confirmed zero misaligned logs, maintaining a 100\% match between the \texttt{input\_tokens} and \texttt{output\_tokens} summations representing the derived \texttt{total\_tokens}.

\section{Cost Aggregation Outcomes (M2)}

The Analytics aggregator continuously rolled raw logs into daily and monthly performance metrics, successfully retaining sub-200ms querying targets:
\begin{itemize}
    \item \textbf{Daily Aggregations:} 552 aggregated baseline records reflecting cost distributions across all active tenants.
    \item \textbf{Monthly Rollups:} 19 finalized monthly billing cycles, with usage tier limit tracking.
\end{itemize}

For example, the peak recorded monthly expenditure was \textbf{\$842.73}, correlated with \texttt{C3\_ENTERPRISE} scaling to over 103 million evaluated tokens during June 2025.

\section{Anomaly Detection Ensemble (M3)}

Integrating the baseline history enabled the M3 Detection engine to apply its Isolation Forest and CUSUM algorithms across the historical distribution. By measuring variances against dynamically calibrated means, the ensemble successfully caught and flagged \textbf{449} discrete anomalies. These records outline instances of acute token deviation, latency abnormalities, or rapid cost accumulation that broke statistical tolerance thresholds.

\section{Predictive Forecasting (M6)}

Utilizing the required 21-day historical baseline per tenant, the statistical forecasting engine successfully projected future load patterns. The system rendered \textbf{90} specific 30-day projection vectors, isolating upper and lower confidence intervals (optimistic vs. pessimistic horizons) to preemptively calculate the probability of a tenant exceeding their assigned monthly budget caps.

\section{Discussion}

The seamless ingestion, evaluation, and aggregation of the six-month dataset validates the fundamental scalability of the modular monolith architecture. By ensuring that no logical inconsistencies or missing value states occurred across the full simulation, the M1--M7 pipeline exhibits enterprise-grade stability.

The data further demonstrates that governance evaluation logic remains effectively decoupled from the detection layer. Because pipeline modules run via synchronized Celery tasks handling aggregation separately from asynchronous background event bus notifications (e.g., \texttt{anomaly.detected}), historical backfills do not bottleneck operational read/write availability. 
